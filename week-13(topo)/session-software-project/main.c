...
void projectmanagement(const Graph *pG, int d[] /** , int job, int start*/)
{
    int rank[MAX_NODE];
    toposort(pG, rank);
    int t[MAX_NODE] = {};
    ...
    // start >= t[job] && start <= T[job] ? puts("YES") : puts("NO");
    for (int u = 1; u <= pG->n + 2; u++)
        if (t[u] == T[u])
            printf("%d\n", u);
}
...
int main()
{
    ...
    projectmanagement(&G, d /**,job, start*/);
    return 0;
}